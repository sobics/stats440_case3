...
```{r echo = F}
#clustering graph
ggplot(gambia, aes(jitter(GREEN), jitter(PHC), color = as.factor(g_cluster$cluster))) + geom_point()
...
Between the two "mice" methods, both imputations schemes were similar, but the Bayesian linear regression method was slighlty better. For now, we will move forward with this imputation scheme. Depending on the diagnostics of future models, we may opt to try alternative imputation schemes. 
